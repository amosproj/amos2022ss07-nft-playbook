declare module 'solc' {
  ...
  export function compile(arg0: string): string;
   ...
   */
  export type CompileJsonCallback = (
    input: string,
    optimize: boolean,
    readCallbackPtr: number
  ) => string;
   ...
   * @param contextPtr
   */
  export type CompileJsonStandard = (
    input: string,
    callbackPtr: number,
    contextPtr?: number
  ) => string;
  ...
  export interface Wrapper {
    ...
    loadRemoteVersion(
      version: string,
      callback: (error?: Error, solc?: SolJson) => void
    ): void;
